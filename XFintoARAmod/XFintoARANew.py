...
# .uan file location
uanLoc = '/home/suren/Desktop/OSU Research/XFintoARAmod/' # FIX
...
datLoc = '/home/suren/Desktop/OSU Research/XFintoARAmod/' # FIX
...
def readFile(indivNum, freqNum):
	...
	uanData = np.loadtxt(uanName, delimiter=' ', skiprows=18)
	"""
...
for antenna in range(g.NPOP):
	...
	# (NPOP x freq size x angle size x elements)
	uanDat = np.zeros((60, 325, 5))
	for freq in range(numFreq):
		# Read the antenna+1'th antenna for the freq+1'th frequency
		uanDat[freq] = readFile(antenna+1, freq+1) 

	with open("evol_antenna_model_"+str(antenna)+".dat", "w+") as datFile:
		for freq in range(numFreq):
			...
			so we do it "by hand".
			"""
			for r in range(325): # for each row (r for rows)
				...
				for i in range(3):
					writeStr+= str(round(uanDat[freq, r, i+2], 4))+"      "
